...
import { daysInWeek, WEEKEND_DAYS } from '../shared';
import { format, getDay, isSameDay, Locale, parse } from 'date-fns';
import { CALENDAR_DAY_TYPE } from '../../index';
import { CalendarMonthEventsList } from '../../Calendar';
...
export const handleOmittedDays = ({
    ...
    let firstDayOfMonth = (getDay(daysToRender[0]) as number) - (locale?.options?.weekStartsOn as number);
    firstDayOfMonth = firstDayOfMonth >= 0 ? firstDayOfMonth : firstDayOfMonth + 7;
...
type MonthlyBodyProps<DayData> = {
    ...
    */
    omitDays?: number[];
    events: (DayData & CalendarMonthEventsList & any)[];
    children: ReactNode;
    weekendDays: number[];
};
                ...
                {daysToRender.map(day => (
                    <MonthlyBodyContext.Provider
                        ...
                        value={{
                            day,
                            weekendDays,
                            events: typeof events.filter == 'function' ? events.filter(data => isSameDay(typeof data.date === 'string' ? parse(data.date, 'yyyy-mm-dd', new Date()) : data.date, day)) : [],
                        }}
                    >
                        {children}
                    </MonthlyBodyContext.Provider>
...
type MonthlyDayProps<DayData> = {
    ...
    onDayClick?: (event: any, day: any) => void
};
export function MonthlyDay<DayData>({ renderDay, onDayClick }: MonthlyDayProps<DayData>) {
    const _showEvents = false;

    let { day, events, weekendDays } = useMonthlyBody<DayData>();

    const WEEKENDS = weekendDays || WEEKEND_DAYS;

    let dayNumber = format(day, 'd');
    const dayOrderNumber: number = day.getDay();

    const getSpecialClasses = () => {
        let specialClasses = CALENDAR_DAY_TYPE.WORKING_DAY.toString();

        if (WEEKENDS.includes(dayOrderNumber)) {
            specialClasses = CALENDAR_DAY_TYPE.WEEKEND.toString();
        }

        if (events && events[0]) {
            //@ts-ignore
            specialClasses = CALENDAR_DAY_TYPE[events[0].type].toString();
        }

        return specialClasses;
    }

    const onDayClickHandler = (event: any) => {
        const _fn = onDayClick || (() => { });
        _fn(event, day)
    }
    return (
        <div
            title={format(day, 'yyy-MM-dd')}
            className={[
                "rc-h-48",
                "rc-p-2",
                "rc-border-b-2",
        ...
        >
            ...
            {(events.length > 0 && _showEvents) && <ul className="rc-divide-gray-200 rc-divide-y rc-overflow-hidden rc-max-h-36 rc-overflow-y-auto">
                {renderDay(events)}
            </ul>}
        </div>
    );
